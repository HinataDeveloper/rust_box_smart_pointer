// main.rs
// Date: Thu Sep 22 2026
// Educational and Practice Rust Programming Language Code

// Project: Learning Chapter 15
// Goal: ...
// Dependency: Without dependency

// rustc 1.100.0-nightly (1303417c4 2026-09-21)
// binary: rustc
// commit-hash: 1303417c416e1595173d9689e7394c31e136ae95
// commit-date: 2026-09-21
// host: x86_64-unknown-linux-gnu
// release: 1.100.0-nightly
// LLVM version: 23.1.1

// cargo 1.100.0-nightly (495c385d0 2026-09-16)
// release: 1.100.0-nightly
// commit-hash: 495c385d0875c4ba51eb72ea0448a2d4c018b8d4
// commit-date: 2026-09-16
// host: x86_64-unknown-linux-gnu
// libgit2: 1.9.6 (sys:0.21.0 vendored)
// libcurl: 8.21.0-DEV (sys:0.4.90+curl-8.21.0 vendored ssl:OpenSSL/3.6.3)
// ssl: OpenSSL 3.6.3 9 Jun 2026
// os: Fedora 44.0.0 [64-bit]

// Kernel Version: 7.2.6-200.fc44.x86_64
// Firmware Version: 71CN51WW(V1.21)

use box_smart_pointer::User;
use box_smart_pointer::separator;

fn main() {
    println!("\n");

    let raphael: User = User::new(10033, "@Raphael", "321654987");
    let samuel: User = User::new(20036, "@Samuel", "987654");

    println!("Raphael: {}", raphael);
    separator();
    println!("Samuel: {}", samuel);

    // call core::mem::drop directly
    core::mem::drop(raphael);    

    println!("\nThe End ...\n");
}
