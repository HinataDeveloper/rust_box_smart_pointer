...
impl User {
    pub fn new(user_id: u64, username: impl Into<String>, password: impl Into<String>) -> Self {
        Self {
            user_id,
...
impl Drop for User {
    fn drop(&mut self) {
        println!(" -> it's time to say Goodbye and go back home ...");
    }
}
...
impl Display for User {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "[user_id: {}, username: {}, password <user password>]",
